...
/*changesB Press q to open search */

document.addEventListener('keydown', function(e) {
  const searchInput = document.getElementById('topSearchText');
...
function addLoading(type = "add", text = translate("$loading"), circle = 1, force = 0) {
  ...
  if (type === "remove" || type === "forceRemove") {
    // Restore contentWrapper opacity and body overflow in case something left it hidden
...
const fgOpacity = fgColor.setAlpha(0).toRgbString();
...
/*changesB matched colors to stylus theme changesE*/
let defaultColors = `
:root,
body {
  --fg: #16202f!important;
...
/*changesB async function bgColorFromImage(img) REMOVED changesE*/
...
function pageFixes(page) {
  if (page === "anime-manga") {
    ...
    if (svar.editPopup && $('#addtolist a:contains("Edit Details")').length) {
      ...
      /*changesB On title pages, move the edit button to the right edit panel and remove the left edit panel*/

      function movePenToRight() {
    document.querySelectorAll('.di-ib.form-user-episode.ml8').forEach(episode => {
        // Find the pen icon
        let pen = episode.parentElement.querySelector('.fa.fa-pen');

        // Fallback in case the pen is somewhere else
        if (!pen) {
            pen = document.querySelector('.fa.fa-pen');
        }

        if (pen) {
            // Move the pen to the same container as the episode (if not already)
            if (pen.parentElement !== episode.parentElement) {
                pen.remove();
                episode.parentElement.appendChild(pen);
            }

            // Add a small margin to push it slightly to the right
            pen.style.marginLeft = '10px';   // You can change this value (8px, 12px, etc.)
            pen.style.marginRight = '0';     // Clean up any previous right margin if needed
        }
    });
}
      function hideLeftSideElements() {
    const style = document.createElement('style');
    style.textContent = `
        #content > table > tbody > tr > td.borderClass > div.leftside > h2.mt8,
        #content > table > tbody > tr > td.borderClass > div.leftside > .addtolist-block {
            display: none !important;
        }
    `;
    document.head.appendChild(style);
}
hideLeftSideElements();
movePenToRight();
      /* changesE*/

new MutationObserver(movePenToRight).observe(document.body, {
    childList: true,
    subtree: true
});
